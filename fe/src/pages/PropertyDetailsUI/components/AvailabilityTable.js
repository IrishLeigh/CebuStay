...
export default function AvailabilityTable(propertyid) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resData = await axios.get(
          "https://whitesmoke-shark-473197.hostingersite.com/api/getallunit",
          {
            params: {
              propertyid: propertyid,
            },
        ...
        if (resData.data) {
          ...
          console.log(
            "Unit Detail Rooms",
            resData.data.property_unitdetails[0].unitbeds
          );
                    ...
                    <Grid item xs={12} sx={{ mr: 1 }}>
                      <div>
                        <CarouselFadeExample
                          ...
                          photos={unit.images}
                          photoLength={unit.images.length}
                        />
                    ...
                    <Grid container>
                      ...
                      >
                        {unit.unitrooms.map((room) => (
                          ...
                          >
                            <img
                              src={renderRoomImage(room.roomname)}
                              alt={room.roomname}
                              style={{ width: "2.25rem" }}
                            />
                            <Typography
                      ...
                      <Grid
                        item
                        xs={6}
                        sx={{ borderLeft: "1px solid black", p: 1 }}
                      >
                        <Typography
                          variant="body1"
                        ...
                        {Object.values(unit.unitbeds)
                          .flat()
                          .map((bedsData, index) => (
                            <div key={index} style={{ marginBottom: "0.5rem" }}>
                              <Typography variant="body1" sx={{ ml: 1 }}>
                                Bedroom {bedsData.bedroomnum}
                              </Typography>
                              {Object.keys(bedsData.beds).map(
                                (bedType, index) =>
