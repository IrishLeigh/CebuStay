...
export default function EditPartnerVerification({ parentPartnerData }) {
  ...
  useEffect(() => {
    console.log("parentPartnerData:", parentPartnerData);
  }, []);
  useEffect(() => {
    if (parentPartnerData) {
      ...
      // console.log(
      //   "INDIVIDUAL NI:",
      //   parentPartnerData.property_ownership.ownershiptype
      // );
      if (parentPartnerData.property_ownership.ownershiptype === "Individual") {
        setIndividualData(parentPartnerData.property_owner);
      } else if (
        parentPartnerData.property_ownership.ownershiptype === "Company"
      ) {
    ...
    <>
      ...
      >
        ...
        >
          <div>
            <Typography
              sx={{ fontSize: "1.125rem", fontFamily: "Poppins, sans-serif" }}
              mb={4}
            >
              To ensure compliance with legal and regulatory standards, we
              require some information about you and your property.
            </Typography>
            <Typography
              sx={{ fontSize: "1.125rem", fontFamily: "Poppins, sans-serif" }}
              mb={2}
          ...
          <div>
            {hostType === "Individual" && (
              <IndividualHost
                ...
                isEditing={isEditing}
                parentIndividualData={individualData}
              />
            )}
            {hostType === "Company" && (
