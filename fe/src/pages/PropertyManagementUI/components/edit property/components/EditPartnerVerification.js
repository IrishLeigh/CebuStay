...
export default function EditPartnerVerification({
  parentPartnerData,
  onSaveStatusChange,
  propertyid,
}) {
  ...
  const validateAndProceed = () => {
    ...
    const getEmptyFields = (data) => {
      if (Object.keys(data).length === 0) {
        return [
          "No information has been filled out yet. Please complete the required fields before proceeding.",
        ];
      }
    ...
    if (hostType === "Individual") {
      ...
      if (emptyFields.length > 0) {
        alert(
          `Please fill in the following required information for the individual host: ${emptyFields.join(
            ", "
          )}.`
        );
        return;
    ...
    if (hostType === "Company") {
      ...
      if (emptyFields.length > 0) {
        alert(
          `Please provide the following required details for the company host: ${emptyFields.join(
            ", "
          )}.`
        );
        return;
    ...
    const emailPattern =
      /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|org|net|edu|gov|info|io|co)$/;
    const phoneWithCountryCode = `${individualData.countryCode}${
      individualData.PhoneNumber || companyData.PhoneNumber
    }`;
    ...
    const pattern =
      phonePatterns[individualData.countryCode] || /^\+[1-9]\d{1,14}$/; // Default for other countries
    ...
    const dataToSend =
      hostType === "Individual"
        ? { hostType, ...individualData }
        : { hostType, ...companyData };
    // onHostDataChange(dataToSend);
    // openModal();
  ...
  const onSubmitIndividual = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://whitesmoke-shark-473197.hostingersite.com/api/property-owners/${propertyownerid},`,
        {
  ...
  const onSubmitCompany = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://whitesmoke-shark-473197.hostingersite.com/api/property-companies/${companyid}`,
        {
          propertyownershipid: dataToSend.propertyownershipid,
      ...
      const legalPromises = legalRepresentatives.map(async (representative) => {
        try {
          const res2 = await axios.post(
            `https://whitesmoke-shark-473197.hostingersite.com/api/legal-representative/${representative.id}`,
            {
  ...
  return (
    <>
      ...
      >
        ...
        >
          To ensure compliance with legal and regulatory standards, we require
