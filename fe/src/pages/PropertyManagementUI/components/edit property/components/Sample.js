...
export default function Sample({ propertyData, onSaveStatusChange }) {
  ...
  const [sleepingTypes, setSleepingTypes] = useState(
    newUnitRooms.map(() => "room")
  ...
  // Add a new room
  const addOldRoom = () => {
    setUnitRooms((prevRooms) => [...prevRooms, { roomname: "", quantity: 0 }]);
    setHasChanges(true);
  };
  ...
  const handleQuantityChange = (index, value) => {
    const updatedRooms = [...unitRooms];
    if (updatedRooms[index].quantity !== value) {
      // Only mark as changed if quantity is different
      updatedRooms[index].quantity = value;
      setHasChanges(true);
      setUnitRooms(updatedRooms);
    }
      ...
      prevBedrooms.map((bedroom, idx) =>
        idx === index
          ? {
              ...
              beds: {
                ...bedroom.beds,
                [type]: Math.max(0, bedroom.beds[type] - 1),
              },
            }
          : bedroom
      )
    ...
    setUnitBeds((prevBedrooms) =>
      prevBedrooms.map((bedroom, idx) =>
        idx === index
          ? {
              ...bedroom,
              beds: { ...bedroom.beds, [type]: bedroom.beds[type] + 1 },
            }
          : bedroom
      )
    );
    ...
    setNewUnitBeds((prevBeds) =>
      prevBeds.map((bedroom, idx) =>
        idx === index
          ? {
              ...bedroom,
              beds: { ...bedroom.beds, [bedType]: bedroom.beds[bedType] + 1 },
            }
          : bedroom
      )
    );
      ...
      prevBeds.map((bedroom, idx) =>
        idx === index
          ? {
              ...
              beds: {
                ...bedroom.beds,
                [bedType]: Math.max(0, bedroom.beds[bedType] - 1),
              },
            }
          : bedroom
      )
  ...
  const removeRoom = async (index, unitroomid) => {
    ...
    try {
      const res = await axios.post(
        `https://whitesmoke-shark-473197.hostingersite.com/api/deleteunitroom-singleunit/${unitid}`, // Use the correct API URL
        {
  ...
  const validateForm = () => {
    ...
    if (
      newUnitRooms.some(
        (room) => !room.roomname || room.quantity < 0 || room.quantity === 0
      )
    ) {
      setHasError(true);
      errorMessage.push(
        "Room name is required and quantity must be greater than 0"
      );
      setSnackbarMessage(
        "Room name is required and quantity must be greater than 0"
      );
    ...
    if (!isValidUnitBeds) {
      setHasError(true);
      errorMessage.push(
        "Each bedroom must have at least one bed with a quantity greater than 0"
      );
      setSnackbarMessage(
        "Each bedroom must have at least one bed with a quantity greater than 0"
      );
    ...
    if (hasChanges) {
      try {
        ...
        const res_data = await axios.put(
          `https://whitesmoke-shark-473197.hostingersite.com/api/updateunitinfo/${unitid}`,
          {
      ...
      >
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <TextField
            ...
            value={guestCapacity}
            onChange={(e) =>
              handleGuestCapacityChange(Number(e.target.value) || 0)
        ...
        <Typography variant="subtitle1" color="textSecondary" mb={2}>
          You can add and remove available spaces here. If you want to add or
          remove bedspaces proceed to the bedspace configuration.
        </Typography>
        ...
        {unitRooms.map((room, index) => (
          ...
          >
            <TextField
              ...
              sx={{ width: "40%" }}
              disabled={
                ["Bedspace", "Bathroom", "Living Room", "Kitchen"].includes(
                  room.roomname
                ) || !isEditing
              }
              helperText=""
        ...
        {newUnitRooms.map((room, index) => (
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <IconButton
                onClick={() => decrementNewQuantity(index)}
                disabled={["Bedspace"].includes(room.roomname) || !isEditing}
              >
                <RemoveIcon />
              </IconButton>
              <TextField
                value={room.quantity}
                onChange={(e) =>
                  handleNewQuantityChange(index, Number(e.target.value) || 0)
                }
                sx={{ width: "4rem", textAlign: "center" }}
                disabled={["Bedspace"].includes(room.roomname) || !isEditing}
              />
              <IconButton
                onClick={() => incrementNewQuantity(index)}
                disabled={["Bedspace"].includes(room.roomname) || !isEditing}
              >
                <AddIcon />
              </IconButton>
        ...
        <Box mt={4}>
          ...
          <Typography variant="subtitle1" color="textSecondary" mb={2}>
            You can add and remove bedspaces in this section. Please select the
            number of beds for each bedroom or bed area.
          </Typography>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  ...
                  <RadioGroup
                    ...
                    onChange={(e) =>
                      handleSleepingTypeChangeUnitBeds(index, e.target.value)
                    }
                  >
                    <FormControlLabel
                      disabled={!isEditing}
                      ...
                      label={
                        <Box>
                          ...
                          <Typography variant="caption">
                            An enclosed bedroom with walls and a door.
                          </Typography>
                        </Box>
                      }
                    />
                    <FormControlLabel
                      ...
                      label={
                        <Box>
                          ...
                          <Typography variant="caption">
                            An open space with beds but no walls or doors
                            separating it.
                          </Typography>
                        </Box>
                      }
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() => incrementBedQuantity(index, "singleBed")}
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    </IconButton>
                    <Typography sx={{ width: "2rem", textAlign: "center" }}>
                      {bedroom.beds?.singleBed || 0} {/* Safely access beds */}
                      {/* Correctly access the beds object */}
                    </Typography>
                    <IconButton
                      onClick={() => decrementBedQuantity(index, "singleBed")}
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                    </IconButton>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() => incrementBedQuantity(index, "doubleBed")}
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    </IconButton>
                    <Typography sx={{ width: "2rem", textAlign: "center" }}>
                      {bedroom.beds?.doubleBed || 0}{" "}
                      {/* Correctly access the beds object */}
                    </Typography>
                    <IconButton
                      onClick={() => decrementBedQuantity(index, "doubleBed")}
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                    </IconButton>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() => incrementBedQuantity(index, "largeBed")}
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    </IconButton>
                    <Typography sx={{ width: "2rem", textAlign: "center" }}>
                      {bedroom.beds?.largeBed || 0}{" "}
                      {/* Correctly access the beds object */}
                    </Typography>
                    <IconButton
                      onClick={() => decrementBedQuantity(index, "largeBed")}
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                    </IconButton>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() =>
                        incrementBedQuantity(index, "superLargeBed")
                      }
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    ...
                    <Typography sx={{ width: "2rem", textAlign: "center" }}>
                      {bedroom.beds?.superLargeBed || 0}{" "}
                      {/* Correctly access the beds object */}
                    </Typography>
                    <IconButton
                      onClick={() =>
                        decrementBedQuantity(index, "superLargeBed")
                      }
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
            ...
            >
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  ...
                  <IconButton
                    onClick={() => removeNewUnitBed(index)}
                    color="error"
                    disabled={!isEditing}
                  >
                    <DeleteIcon />
                  ...
                  <RadioGroup
                    value={bedroom.sleepingtype}
                    onChange={(e) =>
                      handleSleepingTypeChangeNewUnitBeds(index, e.target.value)
                    }
                  >
                    <FormControlLabel
                      disabled={!isEditing}
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() =>
                        incrementNewBedQuantity(index, "singleBed")
                      }
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    ...
                    <IconButton
                      onClick={() =>
                        decrementNewBedQuantity(index, "singleBed")
                      }
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() =>
                        incrementNewBedQuantity(index, "doubleBed")
                      }
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    ...
                    <IconButton
                      onClick={() =>
                        decrementNewBedQuantity(index, "doubleBed")
                      }
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() => incrementNewBedQuantity(index, "largeBed")}
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    </IconButton>
                    ...
                    <IconButton
                      onClick={() => decrementNewBedQuantity(index, "largeBed")}
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
                    </IconButton>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <IconButton
                      onClick={() =>
                        incrementNewBedQuantity(index, "superLargeBed")
                      }
                      disabled={!isEditing}
                    >
                      <AddIcon />
                    ...
                    <IconButton
                      onClick={() =>
                        decrementNewBedQuantity(index, "superLargeBed")
                      }
                      disabled={!isEditing}
                    >
                      <RemoveIcon />
