...
export default function CompanyHost( {onDataChange, parentData, isEditing, isCancelled}) {
  ...
  useEffect(() => {
    if (parentData) {
      ...
      setData({
        properytyownershipid: parentData.property_ownership?.propertyownershipid || null,
        companyid: parentData.property_company.propertycompanyid || null,
      ...
      setOriginalData({
        ...
        crop: parentData.crop || { x: 0, y: 0 },
        zoom: parentData.zoom || 1,
        legalRepresentatives: formattedRepresentatives || null,
      });  // Set the original data as well
    }
  }, [parentData]);

  useEffect(() => {
    // If isCancelled is true, reset data to originalData
    if (isCancelled) {
...
const handleCountryCodeChange = (id, newCountryCode) => {
 ...
 console.log ("data sa CHILD COMPONENT COMPANY", data);
    ...
    <div style={styles.container}>
        ...
        {data.imageSrc && (
          <>
            ...
            <Box style={styles.imageActions}>
              <Button variant="contained" onClick={handleSaveImage} style={styles.button} disabled ={!isEditing}>
                Save Image
              </Button>
              <Button
                variant="contained"
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md = {6}>
          <TextField id="Street" label="Street" value={data.Street} onChange={handleChange} fullWidth style={styles.formField}  disabled ={!isEditing}/>
        </Grid>
        <Grid item xs={12} md = {6}>
          <TextField id="Barangay" label="Barangay" value={data.Barangay} onChange={handleChange} fullWidth style={styles.formField} disabled ={!isEditing}/>
        </Grid>
      </Grid>
      <Grid container spacing={2}>
       <Grid item xs={12} md = {6}>
          <TextField id="City" label="Municipality / City" value={data.City} onChange={handleChange} fullWidth style={styles.formField} disabled ={!isEditing}/>
        </Grid>
        <Grid item xs={12} md = {6}>
          <TextField type ="number" id="ZipCode" label="Zip Code" value={data.ZipCode} onChange={handleChange} fullWidth style={styles.formField} disabled ={!isEditing}/>
        </Grid>
      </Grid>
      ...
      <Box key={rep.id} style={styles.legalRepContainer}>
        <Typography variant="h6" style={styles.legalRepTitle} disabled ={!isEditing}>
