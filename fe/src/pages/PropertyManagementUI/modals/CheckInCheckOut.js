      ...
      try {
        ...
        if (item.bookingid) {
          response = await axios.get(
            "https://whitesmoke-shark-473197.hostingersite.com/api/getbookerandguest",
            {
              params: { bookingid: id },
            }
          );
    ...
    const fetchProperties = async () => {
      try {
        ...
        const imgResponse = await axios.get(
          "https://whitesmoke-shark-473197.hostingersite.com/api/getallfirstimg"
        );
      ...
      <div className="container flex-row">
        <div className="left-section flex-col p-6 bg-background rounded-lg shadow-md mr-4">
              ...
              <thead>
                <tr>
                  ...
                  {item.unit_type === "Monthly Term" && (
                    <th style={{ fontSize: "0.9rem" }}>Due Date</th>
                  )}
                </tr>
              </thead>
              <tbody>
                <tr>
                  ...
                  {item.unit_type === "Monthly Term" && (
                    <td style={{ fontSize: "0.8rem" }}>
                      {formatDate(editedItem.due_date)}
                    </td>
                  )}
                </tr>
              </tbody>
          ...
          <div className="update-details-container">
            {editedItem.status !== "Cancelled" &&
              editedItem.status !== "Checked out" && <></>}
            ...
            <button
              ...
              style={{
                backgroundColor:
                  editedItem.status !== "Confirmed" ? "gray" : "",
                color: editedItem.status !== "Confirmed" ? "#ffffff" : "",
                cursor:
                  editedItem.status !== "Confirmed" ? "not-allowed" : "pointer",
              }}
            >
              Check In
            </button>
            <button
              ...
              style={{
                backgroundColor:
                  editedItem.status !== "Checked in" ? "gray" : "",
                color: editedItem.status !== "Checked in" ? "#ffffff" : "",
                cursor:
                  editedItem.status !== "Checked in"
                    ? "not-allowed"
                    : "pointer",
              }}
            >
            ...
            <>
              <div>
                ...
                >
                  ...
                  <Typography variant="body1" color="textSecondary">
                    {formatPrice(
                      (item.stay_length / 31) * (item.min_price / 1.12)
                    ) || "N/A"}
                  </Typography>
                </Stack>
            ...
            <>
              <div>
                ...
                >
                  ...
                  <Typography variant="body1" color="textSecondary">
                    {formatPrice(item.stay_length * (item.min_price / 1.12)) ||
                      "N/A"}
                  </Typography>
                </Stack>
                {/* Conditional rendering for Security Deposit */}
