...
export default function PropertyManagementListing() {
    ...
    const [data, setData] = useState([
        { id: '#P123', name: 'Sunset Resort', type: 'Suite', address: '123 Beach St, Miami, FL', amount: '$2000', status: 'Active' },
        { id: '#P456', name: 'Mountain Lodge', type: 'Cabin', address: '456 Mountain Rd, Denver, CO', amount: '$1500', status: 'Inactive' },
    ]);
                    ...
                    <table style={{ width: '100%', textAlign: 'center' }}>
                        <thead style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>
                            <tr>
                                ...
                                <th style={{ padding: '0.5rem 1rem', textAlign: 'center', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Paid Amount</th>
                        ...
                        <tbody>
                            {getData().map(item => (
                                <tr key={item.id} className="table-row">
                                    ...
                                    <td className="table-cell">{item.editable ? <input type="text" value={item.amount} onChange={(e) => setData(data.map(i => i.id === item.id ? { ...i, amount: e.target.value } : i))} /> : item.amount}</td>
                                    ...
                                    if (item.id === editItemId) {
                                        return (
                                            <div key={index} className="edit-item">
                                                ...
                                                <label  className='edit-title'>Paid Amount:</label>
                                                <input type="text" value={item.amount} onChange={(e) => setData(data.map(i => i.id === item.id ? { ...i, amount: e.target.value } : i))} />
